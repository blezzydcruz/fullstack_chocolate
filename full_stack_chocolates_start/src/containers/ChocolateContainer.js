...
const ChocolateContainer = () => {
  ...
  const fetchChocolates = async () => {
    ...
    //  can format data here - filter, removing unnecessary values etc.
    setChocolates(data);
  };
  ...
  const deleteChocolate = async (id) => {
    const response = await fetch("http://localhost:8080/chocolates/" + id, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    });
    const keptChocolates = chocolates.filter(
      (chocolate) => chocolate.id !== parseInt(id)
    );
